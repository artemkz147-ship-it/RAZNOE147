...
func _apply_scientific_scale(actor: Node3D) -> void:
    ...
    var source_length := maxf(bounds.size.x, maxf(bounds.size.y, bounds.size.z))
...
func _build_ui() -> void:
    ...
    latin_label = Label.new()
    latin_label.modulate = Color(1, 1, 1, 0.72)
    latin_label.add_theme_font_size_override("font_size", 17)
    titles.add_child(latin_label)
    era_label = Label.new()
    era_label.modulate = Color(1, 1, 1, 0.62)
    titles.add_child(era_label)

    quality_button = Button.new()
    quality_button.text = "Качество: %s" % _quality_label(quality_manager.current_preset)
    quality_button.custom_minimum_size = Vector2(150, 48)
    ...
    info_panel.offset_left = -430
    info_panel.offset_top = -250
    info_panel.offset_right = -22
    info_panel.offset_bottom = 250
    ...
    margin.add_child(info_text)

func _add_button(parent: Container, text: String, callback: Callable, width: float) -> Button:
...
func _update_labels() -> void:
    name_label.text = str(current_data.get("name_ru", ""))
    latin_label.text = str(current_data.get("scientific_name", ""))
    era_label.text = "%s · %s" % [current_data.get("period_ru", ""), current_data.get("region_ru", "")]
...
func _refresh_action_controls() -> void:
    ...
    if action_button != null:
        ...
        for item in actions:
            if dinosaur_controller.has_action(str(item)):
...
func _action() -> void:
    ...
    for item in actions:
        ...
        if dinosaur_controller.has_action(action_name):
            supported.append(action_name)
    if supported.is_empty():
        return
    dinosaur_controller.play_action(supported.pick_random())
...
func _cycle_quality() -> void:
    var order := ["performance", "high", "cinema"]
    var pos := order.find(quality_manager.current_preset)
    var next_name: String = order[(pos + 1) % order.size()]
    quality_manager.apply(next_name)
    quality_button.text = "Качество: %s" % _quality_label(next_name)
