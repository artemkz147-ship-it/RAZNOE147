...
// The user's most visible complaint in v2 was bad weapon/hand placement. Capture
// every hero in live gameplay so no class can silently ship with a weapon across
// the face, behind the body, or missing entirely.
...
for(let i=0;i<heroIds.length;i++){
  ...
  const data=await openPage(`http://127.0.0.1:4173/?autostart=1&debug-fast=1&map=sunny_meadow&hero=${hero}`,960,540,`qa-hero-${String(i+1).padStart(2,'0')}.png`,1500);
  if(!(Number(data.qaShots)>0)) throw new Error(`Hero weapon did not fire: ${hero}`);
}

await browser.close();
console.log('Real-time WebGL combat, all-hero weapon, portrait and visual QA passed.');
