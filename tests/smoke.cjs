...
function el(kind='el'){listeners[kind]??={};return{style:{},dataset:{},classList:classList(),children:[],nextSibling:null,setAttribute(){},insertBefore(n){this.children.push(n)},appendChild(n){this.children.push(n)},addEventListener(t,f){(listeners[kind][t]??=[]).push(f)},getContext(){return ctx},canGoBack(){return false}}}
...
global.Image=class{constructor(){this.onload=null;this.onerror=null;this.complete=false}set src(v){this._src=v;this.complete=true;if(this.onload)this.onload()}get src(){return this._src}};
...
if(global.UMK3_HD_ATLASES.stats().cached<1)throw new Error('HD atlas did not warm');
tapCanvas();step(3);
...
if(!raf.length)throw new Error('RAF stopped during HD fight');
const g=global.__UMK3_DEBUG__.game;
if(!g.player||!g.enemy)throw new Error('Fighters missing');
global.UMK3_HD_ATLASES.ensure(g.player.data);global.UMK3_HD_ATLASES.ensure(g.enemy.data);
