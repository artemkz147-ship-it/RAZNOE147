...
global.Image=class{constructor(){this.onload=null;this.onerror=null;this.complete=false;this.naturalWidth=1920}set src(v){this._src=v;this.complete=true;if(v&&this.onload)this.onload()}get src(){return this._src}};
...
for(const file of ['boot-guard.js','umk3-data.js','version.js','game.js','raster-runtime.js']){
...
let c=global.__UMK3_RASTER__.cacheStats();
if(c.fighters!==0||c.stages>1)throw new Error(`Heavy assets preloaded on title: ${JSON.stringify(c)}`);
tapCanvas();step(3);
if(global.__UMK3_DEBUG__.game.state!=='select')throw new Error('Title -> select failed');
step(2);c=global.__UMK3_RASTER__.cacheStats();
if(c.fighters!==0)throw new Error(`Select screen loaded fighter atlases: ${JSON.stringify(c)}`);
...
c=global.__UMK3_RASTER__.cacheStats();
if(c.fighters<2||c.fighters>2)throw new Error(`Fight should retain exactly two fighter atlases: ${JSON.stringify(c)}`);
if(c.stages<1||c.stages>1)throw new Error(`Fight should retain exactly one stage: ${JSON.stringify(c)}`);
if(c.portraits!==0)throw new Error(`Fight should release select portraits: ${JSON.stringify(c)}`);
console.log(`SMOKE_REALART_LAZY_OK version=${window.UMK3_DATA.version} state=${g.state} roster=${window.UMK3_DATA.allPlayable.length} stages=${window.UMK3_DATA.stages.length} caches=${JSON.stringify(c)}`);
