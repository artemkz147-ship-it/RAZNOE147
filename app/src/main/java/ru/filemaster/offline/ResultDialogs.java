...
final class ResultDialogs {
    static final String OUTPUT_FOLDER = "Загрузки / ФайлМастер";
    ...
    static void show(Activity activity, String detail, Uri uri) {
        ...
        folder.setOnClickListener(v -> ViewerIntents.openOutputFolder(activity));
        ...
        dialog.setOnShowListener(d -> {
            TextView title = activity.findViewById(activity.getResources().getIdentifier("alertTitle", "id", "android"));
            if (title != null) title.setTypeface(Typeface.DEFAULT, Typeface.BOLD);
        });
        dialog.show();
    }
