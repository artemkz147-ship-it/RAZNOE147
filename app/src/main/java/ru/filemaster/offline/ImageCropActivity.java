...
public class ImageCropActivity extends AppCompatActivity {
    ...
    private void build(){LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setBackgroundColor(Color.rgb(247,248,252));ViewCompat.setOnApplyWindowInsetsListener(root,(v,i)->{Insets s=i.getInsets(WindowInsetsCompat.Type.systemBars()|WindowInsetsCompat.Type.displayCutout());v.setPadding(dp(8)+s.left,dp(6)+s.top,dp(8)+s.right,dp(8)+s.bottom);return i;});LinearLayout top=new LinearLayout(this);top.setGravity(Gravity.CENTER_VERTICAL);TextView back=t("←",30,true);back.setGravity(Gravity.CENTER);back.setOnClickListener(v->finish());top.addView(back,new LinearLayout.LayoutParams(dp(48),dp(48)));LinearLayout names=new LinearLayout(this);names.setOrientation(LinearLayout.VERTICAL);names.addView(t("Кадрирование",24,true));status=t("Открываю изображение…",12,false);names.addView(status);top.addView(names,new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1f));root.addView(top);crop=new CropView(this);crop.setBackgroundColor(Color.rgb(33,35,42));root.addView(crop,new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,0,1f));HorizontalScrollView hsv=new HorizontalScrollView(this);hsv.setHorizontalScrollBarEnabled(false);LinearLayout modes=new LinearLayout(this);
        modes.addView(mode("9:16",()->selectRatio("9:16",9f/16f)));modes.addView(mode("16:9",()->selectRatio("16:9",16f/9f)));modes.addView(mode("1:1",()->selectRatio("1:1",1f)));modes.addView(mode("4:5",()->selectRatio("4:5",4f/5f)));modes.addView(mode("5:4",()->selectRatio("5:4",5f/4f)));modes.addView(mode("3:4",()->selectRatio("3:4",3f/4f)));modes.addView(mode("4:3",()->selectRatio("4:3",4f/3f)));modes.addView(mode("2:3",()->selectRatio("2:3",2f/3f)));modes.addView(mode("3:2",()->selectRatio("3:2",3f/2f)));modes.addView(mode("5:7",()->selectRatio("5:7",5f/7f)));modes.addView(mode("7:5",()->selectRatio("7:5",7f/5f)));modes.addView(mode("A4 ↕",()->selectRatio("A4 портрет",210f/297f)));modes.addView(mode("A4 ↔",()->selectRatio("A4 альбом",297f/210f)));modes.addView(mode("21:9",()->selectRatio("21:9",21f/9f)));modes.addView(mode("9:21",()->selectRatio("9:21",9f/21f)));modes.addView(mode("Свободно",()->{crop.setFree();status.setText("Свободное кадрирование • рамку можно двигать и растягивать");}));modes.addView(mode("Точный размер",this::askExact));hsv.addView(modes);root.addView(hsv,new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(52)));Button save=new Button(this);save.setText("Вырезать выбранную область");save.setAllCaps(false);save.setOnClickListener(v->save());root.addView(save,new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(54)));setContentView(root);}
    private void load(){worker.submit(()->{try{ImageAdvancedTools.Size s=ImageAdvancedTools.size(this,uri);Bitmap p=ImageTools.loadScaled(this,uri,2200);runOnUiThread(()->{crop.setImage(p,s.width,s.height);crop.setRatio(9f/16f);status.setText("Исходник: "+s.width+" × "+s.height+" px • выбран формат 9:16");});}catch(Exception e){runOnUiThread(()->error(e));}});}
    private void selectRatio(String name,float ratio){crop.setRatio(ratio);status.setText("Пропорция "+name+" • рамку можно двигать и менять размер без изменения пропорции");}
