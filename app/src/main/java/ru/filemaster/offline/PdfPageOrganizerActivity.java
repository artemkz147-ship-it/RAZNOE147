...
public class PdfPageOrganizerActivity extends AppCompatActivity {
    ...
    private void build() {
        ...
        TextView note=text("Меняйте порядок, поворачивайте, удаляйте и создавайте копии страниц. Исходный PDF не изменяется.",12,Color.rgb(93,99,112),false);note.setPadding(dp(8),0,dp(8),dp(8));root.addView(note);
    ...
    private void renderList() {
        ...
        for(int pos=0;pos<items.size();pos++){final int index=pos;Item item=items.get(pos);LinearLayout row=new LinearLayout(this);row.setOrientation(LinearLayout.HORIZONTAL);row.setGravity(Gravity.CENTER_VERTICAL);row.setPadding(dp(8),dp(8),dp(6),dp(8));row.setBackgroundColor(Color.WHITE);
            ImageView iv=new ImageView(this);iv.setScaleType(ImageView.ScaleType.CENTER_CROP);iv.setImageBitmap(item.thumb);if(item.rotationDelta!=0)iv.setRotation(item.rotationDelta);row.addView(iv,new LinearLayout.LayoutParams(dp(72),dp(96)));
            LinearLayout center=new LinearLayout(this);center.setOrientation(LinearLayout.VERTICAL);center.setPadding(dp(10),0,dp(6),0);center.addView(text("Страница "+(item.sourceIndex+1),15,Color.rgb(25,28,36),true));center.addView(text("Позиция "+(pos+1)+(item.rotationDelta==0?"":" • поворот "+item.rotationDelta+"°"),12,Color.rgb(93,99,112),false));
