...
public class ImageCompressionActivity extends AppCompatActivity {
    ...
    private void build(){LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setBackgroundColor(Color.rgb(247,248,252));ViewCompat.setOnApplyWindowInsetsListener(root,(v,i)->{Insets s=i.getInsets(WindowInsetsCompat.Type.systemBars()|WindowInsetsCompat.Type.displayCutout());v.setPadding(dp(12)+s.left,dp(8)+s.top,dp(12)+s.right,dp(10)+s.bottom);return i;});LinearLayout top=new LinearLayout(this);top.setGravity(Gravity.CENTER_VERTICAL);TextView back=t("←",30,false);back.setGravity(Gravity.CENTER);back.setOnClickListener(v->finish());top.addView(back,new LinearLayout.LayoutParams(dp(48),dp(48)));LinearLayout tt=new LinearLayout(this);tt.setOrientation(LinearLayout.VERTICAL);tt.addView(t("Оптимизация изображения",24,true));tt.addView(t("Разрешение и пропорции не меняются",13,false));top.addView(tt,new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1f));root.addView(top);ImageView preview=new ImageView(this);preview.setAdjustViewBounds(true);preview.setScaleType(ImageView.ScaleType.FIT_CENTER);preview.setBackgroundColor(Color.WHITE);root.addView(preview,new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,0,1f));worker.submit(()->{try{Bitmap b=ImageTools.loadScaled(this,uri,1800);runOnUiThread(()->preview.setImageBitmap(b));}catch(Exception ignored){}});qLabel=t("Максимум качества • JPEG 100%",16,true);qLabel.setPadding(dp(4),dp(12),dp(4),dp(4));root.addView(qLabel);TextView note=t("Чем ниже процент качества, тем меньше файл. Размер изображения в пикселях остаётся прежним.",12,false);note.setPadding(dp(4),0,dp(4),dp(4));root.addView(note);SeekBar bar=new SeekBar(this);bar.setMax(80);bar.setProgress(80);bar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener(){public void onProgressChanged(SeekBar s,int p,boolean f){quality=20+p;qLabel.setText(quality==100?"Максимум качества • JPEG 100%":"Качество JPEG • "+quality+"%");}public void onStartTrackingTouch(SeekBar s){}public void onStopTrackingTouch(SeekBar s){}});root.addView(bar);Button save=new Button(this);save.setText("Сохранить оптимизированную копию");save.setAllCaps(false);save.setOnClickListener(v->save());root.addView(save,new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(54)));setContentView(root);}
    private void save(){ProgressDialog d=ProgressDialog.show(this,"Доки","Оптимизирую изображение…",true,false);int q=quality;worker.submit(()->{try{Uri out=ImageAdvancedTools.optimize(this,uri,q);runOnUiThread(()->{d.dismiss();new AlertDialog.Builder(this).setTitle("Готово").setMessage("Разрешение сохранено. Качество JPEG: "+q+"%.").setPositiveButton("Открыть",(x,w)->ViewerIntents.open(this,out)).setNeutralButton("Поделиться",(x,w)->ViewerIntents.share(this,out)).setNegativeButton("Закрыть",null).show();});}catch(Exception e){runOnUiThread(()->{d.dismiss();new AlertDialog.Builder(this).setTitle("Не получилось").setMessage(e.getMessage()).setPositiveButton("OK",null).show();});}});}
    private TextView t(String s,int sp,boolean bold){TextView v=new TextView(this);v.setText(s);v.setTextSize(sp);v.setTextColor(Color.rgb(25,28,36));if(!bold)v.setTextColor(Color.rgb(93,99,112));if(bold)v.setTypeface(v.getTypeface(),android.graphics.Typeface.BOLD);return v;}private int dp(int v){return Math.round(v*getResources().getDisplayMetrics().density);}@Override protected void onDestroy(){super.onDestroy();worker.shutdownNow();}
}
