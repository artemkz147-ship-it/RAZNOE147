...
public class SignatureActivity extends AppCompatActivity {
    ...
    static class SignatureView extends View{private final Paint paint=new Paint(Paint.ANTI_ALIAS_FLAG);private final Path path=new Path();private boolean hasInk;SignatureView(android.content.Context context){super(context);paint.setColor(Color.BLACK);paint.setStyle(Paint.Style.STROKE);paint.setStrokeCap(Paint.Cap.ROUND);paint.setStrokeJoin(Paint.Join.ROUND);paint.setStrokeWidth(5f*getResources().getDisplayMetrics().density);}@Override protected void onDraw(Canvas canvas){super.onDraw(canvas);canvas.drawPath(path,paint);}@Override public boolean onTouchEvent(MotionEvent event){float x=event.getX(),y=event.getY();switch(event.getAction()){case MotionEvent.ACTION_DOWN->{path.moveTo(x,y);hasInk=true;invalidate();return true;}case MotionEvent.ACTION_MOVE->{path.lineTo(x,y);invalidate();return true;}case MotionEvent.ACTION_UP->{path.lineTo(x,y);invalidate();return true;}}return super.onTouchEvent(event);}void clear(){path.reset();hasInk=false;invalidate();}boolean hasInk(){return hasInk;}Bitmap toBitmap(){int w=Math.max(getWidth(),1),h=Math.max(getHeight(),1);Bitmap bitmap=Bitmap.createBitmap(w,h,Bitmap.Config.ARGB_8888);Canvas canvas=new Canvas(bitmap);canvas.drawColor(Color.TRANSPARENT);canvas.drawPath(path,paint);return bitmap;}}
}
