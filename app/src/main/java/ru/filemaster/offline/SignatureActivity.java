...
public class SignatureActivity extends AppCompatActivity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState){super.onCreate(savedInstanceState);WindowCompat.setDecorFitsSystemWindows(getWindow(),false);LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(16),dp(12),dp(16),dp(18));root.setBackgroundColor(BG);ViewCompat.setOnApplyWindowInsetsListener(root,(v,insets)->{Insets safe=insets.getInsets(WindowInsetsCompat.Type.systemBars()|WindowInsetsCompat.Type.displayCutout());v.setPadding(dp(16),dp(12)+safe.top,dp(16),dp(18)+safe.bottom);return insets;});LinearLayout top=new LinearLayout(this);top.setGravity(Gravity.CENTER_VERTICAL);TextView back=label("←",30,TEXT,false);back.setGravity(Gravity.CENTER);back.setOnClickListener(v->finish());top.addView(back,new LinearLayout.LayoutParams(dp(52),dp(52)));LinearLayout names=new LinearLayout(this);names.setOrientation(LinearLayout.VERTICAL);names.addView(label("Нарисуйте подпись",25,TEXT,true));TextView small=label("Образец сохранится в «Мои подписи»",14,MUTED,false);small.setPadding(0,dp(2),0,0);names.addView(small);top.addView(names,new LinearLayout.LayoutParams(0,LinearLayout.LayoutParams.WRAP_CONTENT,1f));root.addView(top);TextView hint=label("Подпись хранится только на устройстве. После сохранения её можно повторно вставлять в любые PDF, копировать, менять размер и размещать на разных страницах.",14,MUTED,false);hint.setPadding(dp(4),dp(8),dp(4),dp(14));root.addView(hint);SignatureView pad=new SignatureView(this);GradientDrawable padBg=rounded(Color.WHITE,dp(18));padBg.setStroke(dp(1),Color.rgb(222,225,234));pad.setBackground(padBg);pad.setPadding(dp(10),dp(10),dp(10),dp(10));LinearLayout.LayoutParams padParams=new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,0,1f);padParams.setMargins(0,0,0,dp(14));root.addView(pad,padParams);LinearLayout actions=new LinearLayout(this);TextView clear=action("Очистить",Color.WHITE,TEXT,Color.rgb(227,230,237));clear.setOnClickListener(v->pad.clear());LinearLayout.LayoutParams left=new LinearLayout.LayoutParams(0,dp(54),1f);left.setMargins(0,0,dp(6),0);actions.addView(clear,left);TextView save=action("Сохранить подпись",BLUE,Color.WHITE,BLUE);save.setOnClickListener(v->{if(!pad.hasInk()){Toast.makeText(this,"Сначала нарисуйте подпись",Toast.LENGTH_SHORT).show();return;}try{Bitmap bitmap=pad.toBitmap();File out;try{out=SignatureStore.save(this,bitmap);}finally{bitmap.recycle();}Intent result=new Intent();result.putExtra("signature_path",out.getAbsolutePath());setResult(RESULT_OK,result);finish();}catch(Exception e){Toast.makeText(this,"Не удалось сохранить подпись",Toast.LENGTH_LONG).show();}});LinearLayout.LayoutParams right=new LinearLayout.LayoutParams(0,dp(54),1.55f);right.setMargins(dp(6),0,0,0);actions.addView(save,right);root.addView(actions);setContentView(root);}
