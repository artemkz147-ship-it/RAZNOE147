...
class ExportManager(private val context:Context){private var transformer:Transformer?=null;private var tempFile:File?=null;private var active=false
    fun export(clips:List<VideoClip>,backgroundAudio:AudioTrack?,positionedAudioTracks:List<PositionedAudioTrack>,subtitles:List<SubtitleCue>,subtitleStyle:SubtitleStyle,settings:ExportSettings,onProgress:(Int)->Unit,onDone:(String)->Unit,onError:(String)->Unit){
        ...
        val items=clips.mapIndexed{index,clip->val media=MediaItem.Builder().setUri(clip.uri).setClippingConfiguration(MediaItem.ClippingConfiguration.Builder().setStartPositionMs(clip.trimStartMs).setEndPositionMs(clip.trimEndMs).build()).build();val incoming=clips.getOrNull(index-1)?.let{p->if(p.transitionOut==TransitionType.NONE)null else TransitionSpec(p.transitionOut,p.transitionDurationMs)};EditedMediaItem.Builder(media).setRemoveAudio(clip.muted).setSpeed(ConstantSpeedProvider(clip.speed)).setFrameRate(settings.maxFrameRate).setEffects(Effects(buildClipAudioEffects(clip),buildVideoEffects(context,clip.copy(keyframes=emptyList(),stickers=emptyList()),incoming)+buildEasedKeyframeEffects(clip)+buildDynamicImageStickerEffects(context,clip)+buildAnimatedStickerEffects(clip))).build()}
