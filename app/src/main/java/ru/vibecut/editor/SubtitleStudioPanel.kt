...
private data class SubtitlePreset(val title:String,val style:SubtitleStyle)
...
internal fun SubtitleStudioPanel(cues:List<SubtitleCue>,style:SubtitleStyle,projectCursorMs:Long,onImportSrt:()->Unit,onAddCue:(String)->Unit,onDeleteCue:(String)->Unit,onClear:()->Unit,onStyleChange:(SubtitleStyle)->Unit){
    ...
    SectionCard("Субтитры · студия"){
        ...
        Text("Толщина контура: ${"%.1f".format(style.outlineWidth)}",color=Color.White);Slider(style.outlineWidth.coerceIn(0f,7f),{onStyleChange(style.copy(outlineWidth=it))},valueRange=0f..7f)
        Text("Тень: ${"%.1f".format(style.shadowRadius)}",color=Color.White);Slider(style.shadowRadius.coerceIn(0f,8f),{onStyleChange(style.copy(shadowRadius=it))},valueRange=0f..8f)
...
@Composable private fun ToggleItem(title:String,checked:Boolean,onChange:(Boolean)->Unit){Row(verticalAlignment=Alignment.CenterVertically){Text(title,color=Color.White);Switch(checked,onChange)}}
@Composable private fun ColorChoices(selected:Int,onSelect:(Int)->Unit){Row(Modifier.fillMaxWidth().horizontalScroll(rememberScrollState()),horizontalArrangement=Arrangement.spacedBy(5.dp)){listOf("Белый" to 0xFFFFFFFF.toInt(),"Чёрный" to 0xFF000000.toInt(),"Жёлтый" to 0xFFFFD54F.toInt(),"Красный" to 0xFFFF5252.toInt(),"Розовый" to 0xFFFF59D6.toInt(),"Голубой" to 0xFF40C4FF.toInt(),"Циан" to 0xFF7DF9FF.toInt(),"Зелёный" to 0xFF69F0AE.toInt(),"Оранжевый" to 0xFFFFA726.toInt()).forEach{(n,c)->ChoiceButton(n,selected==c){onSelect(c)}}}}
