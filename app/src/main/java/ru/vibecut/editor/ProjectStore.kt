...
object ProjectStore {
    ...
    private fun ensureMigrated(context: Context) {
        ...
        val raw = context.getSharedPreferences(LEGACY_PREFS, Context.MODE_PRIVATE)
            .getString(LEGACY_KEY, null) ?: return
    ...
    private fun projectFromJson(json: JSONObject, fallbackId: String): SavedProject {
        fun <T> parseArray(name: String, parser: (JSONObject) -> T): List<T> {
            ...
            return buildList {
                for (i in 0 until arr.length()) arr.optJSONObject(i)?.let { add(parser(it)) }
            }
        }
        return SavedProject(
    ...
    private fun clipToJson(c: VideoClip) = JSONObject().apply {
        ...
        put("muted", c.muted)
        put("audioVolume", c.audioVolume.toDouble())
        ...
        put("hue", c.hue.toDouble())
        put("lightness", c.lightness.toDouble())
        put("crop", c.crop.toDouble())
        put("flipHorizontal", c.flipHorizontal)
        put("flipVertical", c.flipVertical)
        put("motion", c.motion.name)
        put("motionStrength", c.motionStrength.toDouble())
        put("colorEffect", c.colorEffect.name)
        put("redScale", c.redScale.toDouble())
        put("greenScale", c.greenScale.toDouble())
        put("blueScale", c.blueScale.toDouble())
        put("maskType", c.maskType.name)
        put("maskSize", c.maskSize.toDouble())
        put("vignette", c.vignette.toDouble())
        put("transitionOut", c.transitionOut.name)
        put("transitionDurationMs", c.transitionDurationMs)
        ...
        put("overlayText", c.overlayText)
        put("textX", c.textX.toDouble())
        put("textY", c.textY.toDouble())
        put("textScale", c.textScale.toDouble())
        put("textRotation", c.textRotation.toDouble())
        put("textColor", c.textColor)
        put("textBackground", c.textBackground)
        put("textBold", c.textBold)
        put("textItalic", c.textItalic)
    }

    private fun clipFromJson(j: JSONObject): VideoClip {
        fun <T> arr(name: String, parser: (JSONObject) -> T): List<T> {
            val a = j.optJSONArray(name) ?: JSONArray()
            return buildList {
                for (i in 0 until a.length()) a.optJSONObject(i)?.let { add(parser(it)) }
            }
        }
        ...
        return VideoClip(
            ...
            trimStartMs = j.optLong("trimStartMs", 0L),
            trimEndMs = j.optLong("trimEndMs", source),
            muted = j.optBoolean("muted", false),
            audioVolume = j.optDouble("audioVolume", 1.0).toFloat(),
            audioFadeInMs = j.optLong("audioFadeInMs", 0L),
            audioFadeOutMs = j.optLong("audioFadeOutMs", 0L),
            rotationDegrees = j.optInt("rotationDegrees", 0),
            ...
            saturation = j.optDouble("saturation", 0.0).toFloat(),
            hue = j.optDouble("hue", 0.0).toFloat(),
            lightness = j.optDouble("lightness", 0.0).toFloat(),
            crop = j.optDouble("crop", 0.0).toFloat(),
            flipHorizontal = j.optBoolean("flipHorizontal", false),
            flipVertical = j.optBoolean("flipVertical", false),
            motion = enumOrDefault(j.optString("motion"), ClipMotion.NONE),
            motionStrength = j.optDouble("motionStrength", 0.14).toFloat(),
            colorEffect = enumOrDefault(j.optString("colorEffect"), ColorEffect.NONE),
            redScale = j.optDouble("redScale", 1.0).toFloat(),
            greenScale = j.optDouble("greenScale", 1.0).toFloat(),
            blueScale = j.optDouble("blueScale", 1.0).toFloat(),
            maskType = enumOrDefault(j.optString("maskType"), MaskType.NONE),
            maskSize = j.optDouble("maskSize", 0.82).toFloat(),
            vignette = j.optDouble("vignette", 0.0).toFloat(),
            transitionOut = enumOrDefault(j.optString("transitionOut"), TransitionType.NONE),
            transitionDurationMs = j.optLong("transitionDurationMs", 650L),
            keyframes = arr("keyframes", ::keyframeFromJson),
            stickers = arr("stickers", ::stickerFromJson),
            overlayText = j.optString("overlayText", ""),
            textX = j.optDouble("textX", 0.0).toFloat(),
            textY = j.optDouble("textY", -0.72).toFloat(),
            textScale = j.optDouble("textScale", 0.72).toFloat(),
            textRotation = j.optDouble("textRotation", 0.0).toFloat(),
            textColor = j.optInt("textColor", -1),
            textBackground = j.optBoolean("textBackground", true),
            textBold = j.optBoolean("textBold", true),
            textItalic = j.optBoolean("textItalic", false),
        )
    }

    private fun keyframeToJson(k: TransformKeyframe) = JSONObject().apply {
        put("id", k.id); put("timeMs", k.timeMs); put("x", k.x.toDouble()); put("y", k.y.toDouble()); put("scale", k.scale.toDouble()); put("rotation", k.rotation.toDouble())
    }
    private fun keyframeFromJson(j: JSONObject) = TransformKeyframe(
        j.optString("id", UUID.randomUUID().toString()),
        j.optLong("timeMs", 0L),
        j.optDouble("x", 0.0).toFloat(),
        j.optDouble("y", 0.0).toFloat(),
        j.optDouble("scale", 1.0).toFloat(),
        j.optDouble("rotation", 0.0).toFloat(),
    )
    ...
    private fun stickerFromJson(j: JSONObject) = StickerLayer(
        j.optString("id", UUID.randomUUID().toString()),
        j.getString("uri"),
        j.optString("name", "Изображение"),
        j.optDouble("x", 0.0).toFloat(),
        j.optDouble("y", 0.0).toFloat(),
        j.optDouble("scale", 0.35).toFloat(),
        j.optDouble("rotation", 0.0).toFloat(),
        j.optDouble("alpha", 1.0).toFloat(),
    )

    private fun audioToJson(a: AudioTrack) = JSONObject().apply {
        put("uri", a.uri); put("name", a.name); put("volume", a.volume.toDouble())
    }
    private fun audioFromJson(j: JSONObject) = AudioTrack(
        j.getString("uri"), j.optString("name", "Музыка"), j.optDouble("volume", 0.65).toFloat()
    )

    private fun positionedAudioToJson(a: PositionedAudioTrack) = JSONObject().apply {
        put("id", a.id); put("uri", a.uri); put("name", a.name); put("sourceDurationMs", a.sourceDurationMs); put("startAtMs", a.startAtMs); put("volume", a.volume.toDouble())
    }
    private fun positionedAudioFromJson(j: JSONObject) = PositionedAudioTrack(
        j.optString("id", UUID.randomUUID().toString()),
        j.getString("uri"),
        j.optString("name", "Звук"),
        j.optLong("sourceDurationMs", 1L).coerceAtLeast(1L),
        j.optLong("startAtMs", 0L).coerceAtLeast(0L),
        j.optDouble("volume", 0.85).toFloat(),
    )

    private fun subtitleToJson(s: SubtitleCue) = JSONObject().apply {
        put("id", s.id); put("startMs", s.startMs); put("endMs", s.endMs); put("text", s.text)
    }
    private fun subtitleFromJson(j: JSONObject) = SubtitleCue(
        j.optString("id", UUID.randomUUID().toString()),
        j.optLong("startMs", 0L),
        j.optLong("endMs", 2000L),
        j.optString("text", ""),
    )

    private fun subtitleStyleToJson(s: SubtitleStyle) = JSONObject().apply {
        put("fontScale", s.fontScale.toDouble()); put("textColor", s.textColor); put("backgroundColor", s.backgroundColor); put("backgroundEnabled", s.backgroundEnabled); put("verticalPosition", s.verticalPosition.toDouble())
    }
    private fun subtitleStyleFromJson(j: JSONObject) = SubtitleStyle(
        j.optDouble("fontScale", 1.0).toFloat(),
        j.optInt("textColor", -1),
        j.optInt("backgroundColor", 0xB3000000.toInt()),
        j.optBoolean("backgroundEnabled", true),
        j.optDouble("verticalPosition", 0.84).toFloat(),
    )

    private fun exportToJson(e: ExportSettings) = JSONObject().apply {
        put("height", e.height)
        put("maxFrameRate", e.maxFrameRate)
        if (e.aspectRatio == null) put("aspectRatio", JSONObject.NULL) else put("aspectRatio", e.aspectRatio.toDouble())
        put("cropToFill", e.cropToFill)
        put("videoCodec", e.videoCodec.name)
    }
    private fun exportFromJson(j: JSONObject) = ExportSettings(
        j.optInt("height", 1080),
        j.optInt("maxFrameRate", 30),
        if (j.isNull("aspectRatio")) null else j.optDouble("aspectRatio").toFloat(),
        j.optBoolean("cropToFill", false),
        enumOrDefault(j.optString("videoCodec"), VideoCodec.H264),
    )
