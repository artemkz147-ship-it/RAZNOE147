...
const W=1280,H=720,FLOOR=594,S=1.5,PW=1920,PH=1080,FW=360,FH=480,COLS=8,TAU=Math.PI*2;
...
function frameIndex(f,t){
  ...
  if(f.air){const [b,n]=A.jump;const p=clamp((f.vy+800)/1600,0,1);return b+Math.min(n-1,Math.floor(p*n))}
...
function drawFighter(f,t,mini=false){
  if(!f||f.invisible>50)return false;const d=f.activeData||f.data,im=fighterImage(d);if(!im||!fReady.get(d.id))return false;
  const idx=frameIndex(f,t),sx=(idx%COLS)*FW,sy=((idx/COLS)|0)*FH;
  const baseH=mini?276:(d.style==='motaro'?350:d.style==='shaokahn'?330:306),baseW=baseH*.75;
...
function portraitCell(d,x,y,w,h){const im=portraitImage(d);if(im&&pReady.get(d.id)){ctx.save();ctx.beginPath();ctx.rect(x,y,w,h);ctx.clip();ctx.drawImage(im,x,y,w,h);ctx.restore();return true}return false}
function select(){
  ...
  for(let i=0;i<list.length;i++){
    ...
    if(!portraitCell(d,x+9,y+9,cw-18,ch-34)){const f=fake(d,x+cw/2,y+96);ctx.save();ctx.translate(f.x,f.y);ctx.scale(.38,.38);f.x=0;f.y=0;drawFighter(f,game.clock,true);ctx.restore()}
...
let switched=false;function render(){ctx.setTransform(S,0,0,S,0,0);ctx.clearRect(0,0,W,H);if(game.state==='title')title();else if(game.state==='select')select();else if(game.state==='tower')tower();else if(game.state==='fight')fight();else if(game.state==='ending')simple('EARTHREALM IS FREE',15);else if(game.state==='gameover')simple('YOU HAVE BEEN DEFEATED',6);else{ctx.fillStyle='#050506';ctx.fillRect(0,0,W,H)}if(!switched){const base=document.getElementById('hd-game');if(base)base.style.opacity='0';const original=document.getElementById('game');if(original)original.style.opacity='0';switched=true}requestAnimationFrame(render)}
for(const d of DATA.allPlayable){fighterImage(d);portraitImage(d)}for(const s of DATA.stages)stageImage(s);requestAnimationFrame(render);
window.__UMK3_RASTER__={canvas,fighters,portraits,stages,fReady,pReady,sReady,drawFighter,drawStage,frameIndex};
})();
