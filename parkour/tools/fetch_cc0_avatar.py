...
try:
    with tempfile.TemporaryDirectory(prefix='vertical-avatar-') as temp:
        archive = os.path.join(temp, 'animations.zip')
        request = urllib.request.Request(
            URL,
            headers={
                'User-Agent': 'Mozilla/5.0 VerticalParkourBuild/1.0',
                'Referer': 'https://opengameart.org/content/universal-animation-library-2',
            },
        )
        print('Downloading Quaternius Universal Animation Library 2 CC0...')
        ...
        if not candidates:
            raise RuntimeError('No GLB files found in animation archive')

        candidates.sort(key=lambda item: (item[0], item[1]), reverse=True)
        score, size, name, data, clips, meshes, skins = candidates[0]
        target = os.path.join(OUT, 'parkour_performer.glb')
        with open(target, 'wb') as file:
            file.write(data)
        write_manifest('assets3d/character/parkour_performer.glb', clips)
        with open(os.path.join(OUT, 'LICENSE_CC0.txt'), 'w', encoding='utf-8') as file:
            file.write(
                ...
                'Redistribution source used by build: https://opengameart.org/content/universal-animation-library-2\n'
            )
        preview = ', '.join(clips[:18])
        print(f'Prepared animated CC0 avatar: {os.path.basename(name)}; {meshes} meshes, {skins} skins, {len(clips)} clips, {size / 1024 / 1024:.1f} MB')
        print(f'Animation sample: {preview}')
except Exception as error:
    print(f'WARNING: CC0 avatar fetch failed: {error}')
    write_manifest()
